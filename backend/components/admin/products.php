...
if(isset($_GET['delete'])){
   ...
   $delete_product_image = $conn->prepare("SELECT * FROM tbl_product WHERE product_id = ?");
   $delete_product_image->execute([$delete_id]);
   $fetch_delete_image = $delete_product_image->fetch(PDO::FETCH_ASSOC);
   unlink('../display/uploads'.$fetch_delete_image['image']);
   $delete_product = $conn->prepare("DELETE FROM tbl_product WHERE product_id = ?");
   $delete_product->execute([$delete_id]);
   $delete_cart = $conn->prepare("DELETE FROM tbl_cart WHERE pid = ?");
...
<section class="show-products" style="padding-top: 0;">
   ...
   <?php
      $show_products = $conn->prepare("SELECT p.*, c.category_name AS category_name 
      FROM tbl_product p
   ...
   ?>
   <div class="box">
      <img src="../display/uploads/<?= $fetch_products['image']; ?>" alt="">
      <div class="flex">
